...
class Philosopher(Process):
    ...
    #     while is_locked(self.dinner.forks[s]):
    #         s = randrange(self.dinner.how_many_seats)
...
class Dinner:
    def __init__(self, guests_list: list, how_many_books: int, controlled=True):
        ...
        self.waiter = Semaphore(len(guests_list) - 1) if controlled else None
        self.how_many_books = how_many_books
        self.books = [Lock() for i in range(how_many_books)]

    def lets_get_it_started(self):
        print("{}   ZACZYNAMY UCZTĘ   -   procesy".format(datetime.now().strftime("%H:%M:%S.%f")))
        print()
        t1 = perf_counter()
        philosophers = []
        ...
        print()
        print("Czas trwania uczty:", t2 - t1, "sekund   -   procesy")
